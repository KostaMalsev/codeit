...
// 1. First, let's create a function to handle showing a dropdown menu from the + button

function showAddOptionsMenu() {
  // Create a small dropdown menu
  const addOptionsMenu = document.createElement('div');
  addOptionsMenu.className = 'menu add-options-menu';
  addOptionsMenu.style.position = 'fixed';

  // Add options to the menu
  addOptionsMenu.innerHTML = `
    <div class="icon new-file">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon" height="24" viewBox="0 0 24 24" width="24">
        <path d="M0 0h24v24H0z" fill="none"></path>
        <path d="M6 2c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8.83c0-.53-.21-1.04-.59-1.41l-4.83-4.83c-.37-.38-.88-.59-1.41-.59H6zm7 6V3.5L18.5 9H14c-.55 0-1-.45-1-1z" fill="currentColor"></path>
      </svg>
      <a>New File</a>
    </div>
    <div class="icon new-folder">
      <svg xmlns="http://www.w3.org/2000/svg" class="icon" height="24" viewBox="0 0 24 24" width="24">
        <path d="M0 0h24v24H0z" fill="none"></path>
        <path d="M20 6h-8l-2-2H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 1.99 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2zm0 12H4V8h16v10zm-8-4h2v2h2v-2h2v-2h-2v-2h-2v2h-2v2z" fill="currentColor"></path>
      </svg>
      <a>New Folder</a>
    </div>
  `;
  ...
  // Position the menu
  moveElToEl(addOptionsMenu, addButton, 13, { top: 10, left: -80 });
  document.body.appendChild(addOptionsMenu);

  // Add event listeners
  const newFileOption = addOptionsMenu.querySelector('.new-file');
  const newFolderOption = addOptionsMenu.querySelector('.new-folder');

  newFileOption.addEventListener('click', () => {
    document.body.removeChild(addOptionsMenu);
    createNewFileInHTML();
  });

  newFolderOption.addEventListener('click', () => {
    document.body.removeChild(addOptionsMenu);
    createNewFolderInHTML();
  });

  // Close menu when clicking elsewhere
  function closeMenuOnClick(e) {
    if (!addOptionsMenu.contains(e.target) && e.target !== addButton) {
      document.body.removeChild(addOptionsMenu);
  ...
  setTimeout(() => {
    addOptionsMenu.classList.add('visible');
  }, 10);
}

// Function to create a new folder in HTML

function createNewFolderInHTML() {
  // If not already adding a new folder or file
  if (!fileWrapper.querySelector('.focused')) {
    // If intro screen is visible, remove it
    if (fileWrapper.querySelector('.intro')) {
      fileWrapper.querySelector('.intro').remove();
    ...
    folderEl.classList = 'item folder selected focused hidden';
    ...
    // Add new folder to DOM
    fileWrapper.prepend(folderEl);

    // Focus folder
    folderEl.querySelector('.name').focus();
    folderEl.scrollIntoViewIfNeeded();

    // Add push button event listener
    const pushWrapper = folderEl.querySelector('.push-wrapper');

    folderEl.querySelector('.name').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        onNextFrame(pushNewFolderInHTML);
      } else if (e.key === 'Escape') {
        e.preventDefault();
        folderEl.blur();
    ...
    pushWrapper.addEventListener('click', pushNewFolderInHTML);
    ...
    async function pushNewFolderInHTML() {
      if (folderEl.classList.contains('focused')) {
        const dialogResp = await checkPushDialogs();
        if (dialogResp === 'return') return;

        // Play push animation
        playPushAnimation(folderEl.querySelector('.push-wrapper'));

        // Disable pushing folder from HTML
        folderEl.classList.remove('focused');

        // Make folder name uneditable
        folderEl.querySelector('.name').setAttribute('contenteditable', 'false');
        folderEl.querySelector('.name').blur();

        // Validate folder name
        let folderName = folderEl.querySelector('.name').textContent.replaceAll('\n', '');

        // If folder name is empty, use default name
        if (folderName === '') folderName = 'new-folder';

        // Replace all special chars in name with dashes
        const specialChars = validateString(folderName);
        if (specialChars) {
          specialChars.forEach(char => { folderName = folderName.replaceAll(char, '-') });
        }

        // Check if folder already exists with this name
        let nameIndex = 1;
        while (folderNameExists(folderName)) {
          // If folder already has a differentiating number, remove it
          if (nameIndex !== 1) folderName = folderName.slice(0, -('-' + nameIndex).length);

          // Add a differentiating number to folder name
          folderName = folderName + '-' + nameIndex;
          nameIndex++;
        }

        function folderNameExists(name) {
          const folders = fileWrapper.querySelectorAll('.item.folder');
          for (let i = 0; i < folders.length; i++) {
            const folderElem = folders[i];
            const currFolderName = folderElem.querySelector('.name').textContent;
            if (folderEl !== folderElem && name === currFolderName) {
              return true;
            }
          }
          return false;
    ...
    // If already adding a new folder, focus it
    const newFolder = fileWrapper.querySelector('.item.focused');
    const newFolderName = newFolder.querySelector('.name');
    selectAllCaret(newFolderName);
    newFolder.scrollIntoViewIfNeeded();
  }
}

// Function to create a folder in Git by creating a placeholder file in it
async function createFolderInGit(folderName, placeholderFileName, commitMessage) {
  // Map tree location
  const [user, repo, contents] = treeLoc;
  const [repoName, branch] = repo.split(':');
...
// Update the add button click handler to show the menu when in a repository
// and directly create a repo when in the repository list

addButton.addEventListener('click', () => {
  // If navigating in repository
  if (!header.classList.contains('out-of-repo')) {
    // Show dropdown menu with file/folder options
    showAddOptionsMenu();
  } else {
    // Create new repo
    createNewRepoInHTML();
...
// Add this in filebrowser.js after the addButton click event listener
function showAddOptionsMenu() {
  // Create a small dropdown menu
  const addOptionsMenu = document.createElement('div');
  addOptionsMenu.className = 'menu add-options-menu';
...
// 2. Now, let's create a function to create a new folder in HTML
...
// 3. Function to create a folder in Git by creating a placeholder file in it
async function createFolderInGit(folderName, placeholderFileName, commitMessage) {
  // Map tree location
  const [user, repo, contents] = treeLoc;
  const [repoName, branch] = repo.split(':');
